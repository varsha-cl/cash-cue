...
import { getPGliteInstance, clearPGliteInstance } from "./utils/pglite-singleton";
  ...
  persist(
    immer((set, get) => ({
      ...
      create: async (data) => {
        ...
        const postgres = getPGliteInstance(data.name);
      ...
      remove: async (name) => {
        ...
        clearPGliteInstance(name);
        removeIDBItem(postgreIDBName(name));
      },

      connect: async (name) => {
        const postgres = getPGliteInstance(name);
